...
for i=[0:numFiles-1]
    ...
    [~,shp_p] = plot_nongaussian_surface(x_pf(:,1:2),P_pf,[0.30,0.92,0.99925],0.3,p);
    [~,shp_v] = plot_nongaussian_surface(x_pf(:,3:4),P_pf,[0.30,0.92,0.99925],0.3,p);
...
for i=[0:numFiles-1]
    ...
    [~,shp_p] = plot_gaussian_ellipsoid(xesti_ukf(1:2,:),Sesti_ukf(1:2,1:2),sd,p);
    [~,shp_v] = plot_gaussian_ellipsoid(xesti_ukf(3:4,:),Sesti_ukf(3:4,3:4),sd,p);
    shps_ukf{count} = {shp_p, shp_v}; 
...
for i=[0:numFiles-1]
    ...
    [~,shp_p] = plot_gaussian_ellipsoid(xesti_enkf(1:2,:),Sesti_enkf(1:2,1:2),sd,p);
    [~,shp_v] = plot_gaussian_ellipsoid(xesti_enkf(3:4,:),Sesti_enkf(3:4,3:4),sd,p);
    shps_enkf{count} = {shp_p, shp_v}; 
